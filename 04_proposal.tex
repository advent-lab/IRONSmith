...
IRONSmith addresses the NPU programming barrier by replacing the text-based IRON Python workflow with a visual, interactive canvas (Figure \ref{fig:gui}). Users design NPU dataflow applications visually on a canvas and then generate executable IRON Python.  
...
IRONSmith is architected with scalability in mind - there is a frontend (the visual user-facing canvas) and a backend (the compiler that lowers the visual design into code). IRONSmith currently supports AMD Ryzen AI Strix and Phoenix NPU devices.
