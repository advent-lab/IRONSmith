...
def add_activate_test_jit(A, B, D):
    ...
    externalfunc1 = ExternalFunction(
        name="eltwise_add_bf16_scalar", source_file="./mlir-aie/aie_kernels/aie2/add.cc", arg_types=[worker_chunk_ty, worker_chunk_ty, worker_chunk_ty], include_dirs=["/home/mliraie/mlir-aie/aie_kernels/aie2"]
    )

    externalfunc2 = ExternalFunction(
        name="bf16_relu", source_file="./mlir-aie/aie_kernels/aie2/relu.cc", arg_types=[worker_chunk_ty, worker_chunk_ty], include_dirs=["/home/mliraie/mlir-aie/aie_kernels/aie2"]
    )
...
def main():
    ...
    expected = np.maximum(A.numpy().astype(np.float32) + B.numpy().astype(np.float32), 0).astype(bfloat16)
    if np.allclose(D.numpy().astype(np.float32), expected.astype(np.float32), atol=1e-2):
        print("PASS: D matches relu(A + B)")
    else:
        print("FAIL: D does not match relu(A + B)")
